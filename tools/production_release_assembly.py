...
REPORT_NAME="release-assembly-report.json"; PROOF_REPORT_NAME="portable-release-proof.json"; REPORT_SCHEMA="antechkids.robostudio.production-release-assembly"; REPORT_SCHEMA_VERSION=3; _VERSION_RE=re.compile(r"^[A-Za-z0-9][A-Za-z0-9._+-]*$")
class ProductionReleaseAssemblyError(RuntimeError):pass
@dataclass(frozen=True)
class ProductionReleaseInputs:
    executable:Path; runtime_resources:Path; version_file:Path; source_revision:str; runtime_bin:Path; runtime_platformio:Path; compiler_root:Path|None=None; frontend_root:Path|None=None
...
def validate_inputs(inputs:ProductionReleaseInputs,output_root:Path)->str:
    ...
    compiler=_require_directory(inputs.compiler_root or _default_compiler_root(),"application-owned compiler"); frontend=_require_directory(inputs.frontend_root or _default_frontend_root(),"RoboSim frontend")
    ...
    for source in (_resolve(inputs.executable).parent,_resolve(inputs.runtime_resources),_resolve(inputs.runtime_bin),_resolve(inputs.runtime_platformio),compiler,frontend):
...
def assemble_release(inputs:ProductionReleaseInputs,output_root:Path)->dict[str,object]:
    ...
    try:
        ...
        report={"schema":REPORT_SCHEMA,"schema_version":REPORT_SCHEMA_VERSION,"status":"PASS","portable":True,"runtime_model":"application-owned","target_machine_host_toolchain_required":False,"artifact_model":"RoboStudio + Compiler","host_prerequisites_packaged":True,"compiler":"compiler/main.py","compiler_contract":"compiler/robostudio_bridge.py","frontend":"compiler/frontend","bundled_python":"runtime/bin/python.exe","bundled_platformio":"runtime/platformio","application":distribution.application,"application_version":distribution.application_version,"source_revision":inputs.source_revision,"distribution":str(distribution.distribution_root),"artifact":str(release.artifact),"artifact_sha256":release.sha256,"release_manifest":str(release.manifest),"release_provenance":str(provenance),"portable_proof_report":str(proof_report),"proof":proof_payload}; report_path.write_text(json.dumps(report,indent=2)+"\n",encoding="utf-8"); return report
    except Exception as exc:
        if isinstance(exc,ProductionReleaseAssemblyError):raise
        raise ProductionReleaseAssemblyError(str(exc)) from exc
def main()->int:
    p=argparse.ArgumentParser(description="Build a production RoboStudio release with application-owned Python, PlatformIO, compiler, and RoboSim frontend"); p.add_argument("--executable",required=True,type=Path); p.add_argument("--runtime-bin",required=True,type=Path); p.add_argument("--runtime-platformio",required=True,type=Path); p.add_argument("--frontend-root",type=Path); p.add_argument("--compiler-root",type=Path); p.add_argument("--runtime-resources",required=True,type=Path); p.add_argument("--version-file",type=Path,default=repository_root()/"VERSION"); p.add_argument("--source-revision",type=str); p.add_argument("--output",type=Path,default=repository_root()/"releases"/"production"); a=p.parse_args()
    try:r=assemble_release(ProductionReleaseInputs(a.executable,a.runtime_resources,a.version_file,_source_revision(a.source_revision),a.runtime_bin,a.runtime_platformio,a.compiler_root,a.frontend_root),a.output)
    except ProductionReleaseAssemblyError as exc:print(f"RSD-23 production release assembly: FAIL: {exc}",file=sys.stderr);return 1
    print("RSD-23 production release assembly: PASS");print(f"Artifact: {r['artifact']}");print(f"SHA-256: {r['artifact_sha256']}");print("Bundled Python: runtime/bin/python.exe");print("Bundled PlatformIO: runtime/platformio");return 0
