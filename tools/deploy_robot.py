...
def user_device_config_header()->Path:
 """Return the canonical external hardware feature header generated by RoboStudio."""
 return runtime_paths.user_data_root()/"generated"/"generated_device_config.h"

def apply_user_hardware_config(workspace:Path)->Path|None:
 """Overlay user hardware state only into the writable staged firmware copy."""
 header=user_device_config_header()
 if not header.is_file():return None
