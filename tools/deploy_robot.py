...
def user_device_config_header()->Path:
 """Return the canonical mutable hardware feature header for every runtime mode."""
 return hardware_feature_config.generated_header_path()

def apply_user_hardware_config(workspace:Path)->Path:
 """Regenerate from hardware.json and overlay only into the staged firmware copy.

 A generated header is derived state, never a source of truth. Regenerating here
 prevents a stale header left by a previous EXE/source session from overriding
 the current hardware.json selection.
 """
