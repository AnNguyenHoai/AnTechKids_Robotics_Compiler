...
def _run(command: list[str], *, cwd: Path, env: dict[str, str], timeout: float) -> subprocess.CompletedProcess[str]:
    try:
        ...
        raise ProductionFirmwareE2EError(
            f"Production firmware build failed with exit code {exc.returncode}: {detail}"
        ) from exc
...
def build_production_firmware(
    artifact: Path,
    *,
    environment: str = "esp32dev",
    header: Path | None = None,
    project_name: str = "production-e2e",
    timeout: float = DEFAULT_TIMEOUT,
    host_environment: dict[str, str] | None = None,
) -> ProductionFirmwareE2EResult:
    ...
    with tempfile.TemporaryDirectory(prefix="robostudio-production-firmware-e2e-") as td:
        ...
        if not firmware.is_file() or firmware.stat().st_size <= 0:
            raise ProductionFirmwareE2EError(
                f"PlatformIO exited successfully but firmware artifact is missing or empty: {firmware}"
            )
        ...
        return ProductionFirmwareE2EResult(
            "PASS",
            str(artifact),
            environment,
            str(firmware.relative_to(workspace_root)),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build firmware using only the production ZIP runtime")
    ...
    try:
        result = build_production_firmware(
            args.artifact,
