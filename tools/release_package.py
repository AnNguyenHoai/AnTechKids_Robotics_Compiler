...
def _archive_entries(root: Path) -> tuple[list[tuple[Path, str]], list[str]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if not _safe_member(relative): raise ReleasePackageError(f"Release contains forbidden path: {relative}")
        if path.is_dir():
            if not any(path.iterdir()): directories.append(relative.rstrip("/") + "/")
        elif path.is_file(): files.append((path, relative))
    return files, directories

def _zip_info(name: str, *, directory: bool = False) -> zipfile.ZipInfo:
    info = zipfile.ZipInfo(name, DETERMINISTIC_ZIP_TIMESTAMP)
    info.create_system = 3; info.create_version = 20; info.extract_version = 20; info.flag_bits = 0x800; info.compress_type = zipfile.ZIP_DEFLATED
    info.external_attr = (0o40775 << 16 | 0x10) if directory else (0o100644 << 16)
    return info

def _write_deterministic_zip(artifact: Path, files, directories) -> None:
    with zipfile.ZipFile(artifact, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for relative in directories: archive.writestr(_zip_info(relative, directory=True), b"", compress_type=zipfile.ZIP_DEFLATED, compresslevel=9)
        for source, relative in files: archive.writestr(_zip_info(relative), source.read_bytes(), compress_type=zipfile.ZIP_DEFLATED, compresslevel=9)

def build_release(distribution_root: Path, artifact: Path) -> ReleaseArtifact:
    root, artifact = Path(distribution_root).resolve(), Path(artifact).resolve()
    if not root.is_dir(): raise ReleasePackageError(f"Distribution directory not found: {root}")
    if artifact == root or root in artifact.parents: raise ReleasePackageError("Release artifact must not be created inside the distribution directory.")
    try:
        distribution_manifest = distribution_package.validate_distribution_manifest(root / distribution_package.DISTRIBUTION_MANIFEST)
        production = distribution_manifest.get("production_boundary") is True
        if production: production_artifact_boundary.validate_distribution_root(root)
        else:
            from tools import runtime_preflight
            runtime_preflight.validate_distribution(root)
    except Exception as exc: raise ReleasePackageError(f"Distribution validation failed: {exc}") from exc
    entries, directories = _archive_entries(root)
    if not entries: raise ReleasePackageError("Distribution contains no release files")
...
def validate_release_artifact(artifact: Path, manifest: Path | None = None) -> dict:
    artifact, manifest_path = Path(artifact).resolve(), (Path(manifest).resolve() if manifest else Path(artifact).resolve().with_name(RELEASE_MANIFEST))
    if not artifact.is_file(): raise ReleasePackageError(f"Release artifact not found: {artifact}")
    if not manifest_path.is_file(): raise ReleasePackageError(f"Release manifest not found: {manifest_path}")
    try: data = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc: raise ReleasePackageError(f"Unable to load release manifest: {manifest_path}") from exc
    if data.get("schema") != RELEASE_SCHEMA or data.get("schema_version") != RELEASE_SCHEMA_VERSION: raise ReleasePackageError("Unsupported release manifest schema")
    if _sha256(artifact) != data.get("artifact_sha256"): raise ReleasePackageError("Release artifact checksum mismatch")
    ...
    if compatibility.release_schema_version != RELEASE_SCHEMA_VERSION: raise ReleasePackageError(f"Release is incompatible with release schema {RELEASE_SCHEMA_VERSION}")
    ...
    try:
        with zipfile.ZipFile(artifact, "r") as archive:
            ...
            if len(names) != len(set(names)): raise ReleasePackageError("Release artifact contains duplicate paths")
            file_members = [item for item in members if not item.is_dir()]; file_names = [item.filename for item in file_members]
            if set(file_names) != set(expected): raise ReleasePackageError("Release artifact file list does not match release manifest")
            for item in members:
                if not _safe_member(item.filename.rstrip("/")): raise ReleasePackageError(f"Release artifact contains unsafe path: {item.filename}")
                if item.is_dir(): continue
                payload = archive.read(item); _reject_forbidden_text(payload, item.filename); entry = expected[item.filename]
                if len(payload) != entry.get("size"): raise ReleasePackageError(f"Release file size changed: {item.filename}")
                if hashlib.sha256(payload).hexdigest() != entry.get("sha256"): raise ReleasePackageError(f"Release file checksum mismatch: {item.filename}")
    except zipfile.BadZipFile as exc: raise ReleasePackageError(f"Invalid release ZIP: {artifact}") from exc
    return data

def validate_release_compatibility(manifest: dict, **kwargs) -> release_compatibility.Compatibility:
    try: return release_compatibility.validate_compatibility(manifest, **kwargs)
    except release_compatibility.ReleaseCompatibilityError as exc: raise ReleasePackageError(str(exc)) from exc

def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description="Build and validate a portable RoboStudio release ZIP")
    parser.add_argument("--distribution", required=False, type=Path); parser.add_argument("--output", required=False, type=Path); parser.add_argument("--validate", action="store_true")
    args = parser.parse_args()
    if args.validate:
        if args.output is None: parser.error("--output is required with --validate")
        validate_release_artifact(args.output); return 0
    if args.distribution is None or args.output is None: parser.error("--distribution and --output are required unless --help is used")
    build_release(args.distribution, args.output); return 0
