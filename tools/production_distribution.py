...
RUNTIME_BIN_NAME = "runtime-bin"
RUNTIME_PLATFORMIO_NAME = "runtime-platformio"
...
def _contract_source(compiler: Path) -> Path:
    root = compiler / CONTRACT_ENTRY_NAME
    nested = compiler / "compiler" / CONTRACT_ENTRY_NAME
    if root.is_file():
        return root
    if nested.is_file():
        return nested
    raise ProductionDistributionError(
        f"Application-owned compiler contract is missing: expected {root} or {nested}"
    )
...
def _validate_runtime_bin(runtime_bin: Path) -> None:
    ...
    if any(part.lower() in {".venv", "penv", ".pio", ".git", "__pycache__", ".pytest_cache"}
           for path in runtime_bin.rglob("*") for part in path.relative_to(runtime_bin).parts):
...
def _validate_runtime_platformio(runtime_platformio: Path) -> None:
    ...
    if any(part.lower() in {".venv", "penv", ".pio", ".git", "__pycache__", ".pytest_cache"}
           for path in runtime_platformio.rglob("*") for part in path.relative_to(runtime_platformio).parts):
...
def validate_inputs(inputs: ProductionDistributionInputs, output: Path | None = None) -> str:
    ...
    if output is not None:
        ...
        for source in (executable.parent, resources, compiler, frontend, runtime_bin, runtime_platformio):
            try:
...
def _stage_compiler(compiler_root: Path, frontend_root: Path, stage: Path) -> Path:
    ...
    shutil.copytree(
        compiler,
        destination,
        ignore=shutil.ignore_patterns("__pycache__", ".pytest_cache", ".git", ".venv", ".pio", "penv", CONTRACT_ENTRY_NAME),
    )
...
def _stage_runtime(runtime_bin: Path, runtime_platformio: Path, stage: Path) -> tuple[Path, Path]:
    ...
    shutil.copytree(
        _require_directory(runtime_bin, "application-owned portable Python"),
        bin_destination,
        ignore=shutil.ignore_patterns("__pycache__", ".pytest_cache", ".git", ".venv", ".pio", "penv"),
    )
    shutil.copytree(
        _require_directory(runtime_platformio, "application-owned PlatformIO runtime"),
        platformio_destination,
        ignore=shutil.ignore_patterns("__pycache__", ".pytest_cache", ".git", ".venv", ".pio", "penv"),
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"RSD-23 production distribution: FAIL: {exc}", file=sys.stderr)
        return 1
    print("RSD-23 production distribution: PASS")
    print(f"Distribution: {result.distribution_root}")
    print(f"Manifest: {result.manifest}")
    print(f"Python: {result.distribution_root / 'runtime' / 'bin' / 'python.exe'}")
    print(f"PlatformIO: {result.distribution_root / 'runtime' / 'platformio'}")
    return 0
