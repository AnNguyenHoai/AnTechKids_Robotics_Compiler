...
def _copy_tree(source: Path, destination: Path, label: str) -> None:
    ...
    for item in source.iterdir():
        ...
        if item.is_dir():
            if item.name.lower() in {"penv", ".venv", ".pio"}:
                raise DistributionPackageError(f"Host-specific environment is not allowed: {item}")
...
def _file_entries(root: Path) -> list[dict[str, object]]:
    return [
        ...
        if p.is_file() and p.name not in {DISTRIBUTION_MANIFEST, production_artifact_boundary.BOUNDARY_MANIFEST}
    ]
...
def _validate_legacy_runtime_inputs(inputs: DistributionInputs) -> None:
    if inputs.runtime_bin is None or inputs.runtime_platformio is None or inputs.runtime_resources is None:
        raise DistributionPackageError(
            "Legacy distribution assembly requires runtime_bin, runtime_platformio, and runtime_resources"
        )
...
def assemble_distribution(inputs: DistributionInputs, output: Path) -> Path:
    ...
    With ``production_boundary=True`` only application-owned resources are
    copied. Python and PlatformIO inputs are intentionally rejected if passed,
    preventing accidental reintroduction of host-tool bundling.
    """
    ...
    if inputs.production_boundary:
        if inputs.runtime_bin is not None or inputs.runtime_platformio is not None:
            raise DistributionPackageError(
                "Production artifact boundary forbids bundled Python and PlatformIO inputs"
            )
        ...
        try:
            ...
            boundary_manifest = production_artifact_boundary.write_boundary_manifest(output)
        ...
        # The boundary manifest is evidence, not application payload metadata.
        # It is excluded from the distribution file inventory below and is
        # nevertheless shipped in the final ZIP as release metadata.
        _ = boundary_manifest
        portable = False
    ...
    manifest = {
        ...
        "portable": portable,
        "artifact_model": "RoboStudio + Compiler" if inputs.production_boundary else "legacy-runtime",
