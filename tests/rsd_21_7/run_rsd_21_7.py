...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="rsd-21-7-test-") as td:
        ...
        source=base/"sample.py";source.write_text("forward(50)\nwait(100)\nstop()\n",encoding="utf-8");result=production_e2e.evaluate_production_artifact(artifact=artifact,source=source,launch=True,launch_command=[sys.executable,"{app}","--self-test"],compile_command=[sys.executable,"{compiler}","--file","{source}","--output","{output}"]);report=result.to_dict();compiler_output=Path(report["evidence"]["compiler_output"]).as_posix() if report["evidence"]["compiler_output"] else None;check("RoboStudio starts from extracted ZIP",report["robostudio_started"] is True);check("real compiler executes",report["compiler_succeeded"] is True);check("compiler output produced",compiler_output=="e2e-output/program.h");check("E2E PASS",report["status"]=="PASS")
