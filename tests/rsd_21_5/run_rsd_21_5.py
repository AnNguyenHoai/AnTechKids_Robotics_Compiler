...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="rsd-21-5-test-") as td:
        ...
        result=production_e2e.evaluate_production_artifact(artifact=artifact,source=source,launch=True,launch_command=["{python}","{app}","--self-test"],compile_command=["{python}","{compiler}","--source","{source}","--output","{output}"])
