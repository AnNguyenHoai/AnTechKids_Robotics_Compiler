...
def main() -> int:
    with tempfile.TemporaryDirectory() as temp:
        ...
        source_resource.write_text(json.dumps({"schema_version": 1, "kind": "robot_target_capability_profiles", "profiles": []}), encoding="utf-8")
        ...
        with patch.dict(os.environ, {runtime_resources.APPLICATION_HOME_ENV: str(app)}, clear=False):
            with patch.object(runtime_resources, "is_frozen", return_value=True):
                ...
                check("frozen resource resolves under application-owned runtime", resolved == packaged / "robot-isa" / "target_profiles.json")
                check("resource manifest validates", runtime_resources.validate_resource_manifest()["schema_version"] == 1)
                outside = temp_root / "outside.json"
                outside.write_text("{}", encoding="utf-8")
                expect_error("unknown resource cannot escape registry", lambda: runtime_resources.resolve_resource("../../outside"), "Unknown RoboStudio runtime resource")
                (packaged / "robot-isa" / "target_profiles.json").write_text("tampered", encoding="utf-8")
                expect_error("resource checksum drift is rejected", runtime_resources.validate_resource_manifest, "checksum mismatch")
        with patch.object(runtime_resources, "is_frozen", return_value=False):
            with patch.dict(os.environ, {runtime_resources.APPLICATION_HOME_ENV: ""}, clear=False):
                resolved = runtime_resources.resolve_resource("target_profiles")
                check("source development keeps repository resource fallback", resolved == ROOT / "packages" / "robot-isa" / "target_profiles.json")
        with patch.dict(os.environ, {runtime_resources.APPLICATION_HOME_ENV: str(app)}, clear=False):
            with patch.object(runtime_resources, "is_frozen", return_value=True):
                (packaged / "robot-isa" / "target_profiles.json").unlink()
                expect_error("frozen runtime rejects missing resource", lambda: runtime_resources.resolve_resource("target_profiles"), "Required packaged RoboStudio runtime resource is missing")
