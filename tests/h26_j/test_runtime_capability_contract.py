...
from compiler.isa import ISAFunction, ISAInstruction, ISAOperand, OperandKind, ISAProgram
...
class RuntimeCapabilityContractTests(unittest.TestCase):
    ...
    def test_opcode_requirements_are_derived_from_canonical_model(self):
        required = capability_ids_for_opcodes(
            [Opcode.Forward.value, Opcode.SetServo.value]
        )
    ...
    def test_loader_attaches_required_capabilities_to_runtime_program(self):
        binary = self.make_binary(
            ISAInstruction(Opcode.Forward, [ISAOperand.integer(0)])
        )
    ...
    def test_runtime_rejects_missing_capability_before_execution(self):
        program = RuntimeProgram(
            constants=[],
            functions=[],
            instructions=[RuntimeInstruction(Opcode.SetServo, [], 0)],
            required_capabilities=("actuator.servo",),
        )
    ...
    def test_runtime_accepts_program_when_all_capabilities_are_present(self):
        program = RuntimeProgram(
            ...
            required_capabilities=("runtime.control",),
        )
