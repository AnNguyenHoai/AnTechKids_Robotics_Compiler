...
def _request(payload: dict) -> CompileRequest:
    ...
    if not isinstance(target, str) or not target.strip():
        raise ValueError("target must be a non-empty target profile")
    return CompileRequest(source, output, report, source_kind, target.strip())
...
def compile_request(request: CompileRequest) -> CompileResponse:
    ...
    report = (
        Path(request.report).expanduser().resolve()
        if request.report
        else output.parent / "compile_report.json"
    )
    ...
    try:
        ...
        return CompileResponse(
            ...
            request.source_kind,
            str(source),
            str(rewritten) if rewritten else None,
            str(output),
            str(report),
            instruction_count=len(program.instructions),
            target=compiler.target,
...
def run_stdio() -> int:
    try:
        response = compile_request(_request(json.loads(sys.stdin.read())))
    ...
    if args.request:
        try:
            response = compile_request(
                _request(json.loads(args.request.read_text(encoding="utf-8")))
            )
