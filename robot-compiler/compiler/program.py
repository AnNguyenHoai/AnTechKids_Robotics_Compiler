...
class Program:
    ...
    def mark(self, label):
        if label.position is not None:
            raise Exception("Label already marked.")
        label.position = len(self.instructions)

    def add_patch(self, instruction_index, label):
        if label is None:
            raise Exception("Invalid label.")
        self.patches.append(Patch(instruction_index, label))
    ...
    def emit_jump(self, opcode, label):
        ...
        self.emit(opcode)
        self.add_patch(instruction_index, label)

    def emit_jump_if_false(self, opcode, result, label):
        instruction_index = len(self.instructions)
        self.emit(opcode, result)
        self.add_patch(instruction_index, label)

    def emit_jump_if_true(self, opcode, result, label):
        instruction_index = len(self.instructions)
        self.emit(opcode, result)
        self.add_patch(instruction_index, label)
