...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QCheckBox, QPushButton, QGroupBox, QMessageBox
...
class HardwareTab(QWidget):
    ...
    def _build_ui(self):
        ...
        for category in CATEGORY_TITLES:
            ...
            group = QGroupBox(CATEGORY_TITLES[category]); group_layout = QVBoxLayout(group); group_layout.setSpacing(6)
            for device in devices:
                row = QHBoxLayout(); checkbox = QCheckBox(device.display_name); checkbox.setObjectName(f"device_{device.device_id}"); checkbox.setToolTip(device.description)
                self._checkboxes[device.device_id] = checkbox; row.addWidget(checkbox)
                hint = QLabel(device.description); hint.setWordWrap(True); hint.setStyleSheet("color: #666666;"); row.addWidget(hint, 1); group_layout.addLayout(row)
            layout.addWidget(group)
        button_row = QHBoxLayout(); self.save_button = QPushButton("Apply Configuration"); self.save_button.clicked.connect(self.save); button_row.addWidget(self.save_button)
        self.reload_button = QPushButton("Reload"); self.reload_button.clicked.connect(self.reload); button_row.addWidget(self.reload_button); button_row.addStretch(); layout.addLayout(button_row)
        self.status_label = QLabel(""); self.status_label.setWordWrap(True); layout.addWidget(self.status_label); layout.addStretch()
    ...
    def reload(self):
        try:
            ...
            self._set_status(f"Failed to load hardware configuration: {exc}", "red"); QMessageBox.critical(self, "Hardware Configuration", str(exc))

    def save(self):
        try:
            if self._config is None: self._config = self._config_service.load()
            for device_id, checkbox in self._checkboxes.items(): self._config.set_enabled(device_id, checkbox.isChecked())
            self._config_service.save(self._config)
            macro_path = Path(self._macro_service.generate()).expanduser().resolve()
            ...
            self._set_status(artifact_text, "green")
            self.artifacts_generated.emit(artifact_text + "\n")
            self.configuration_saved.emit()
        except Exception as exc:
            self._set_status(f"Failed to save hardware configuration: {exc}", "red"); QMessageBox.critical(self, "Hardware Configuration", str(exc))
